...
const CatFigure = () => {
  ...
  const showSubCats = (e) => {
    const currentLabelText = e.currentTarget.getElementsByTagName('label')[0].innerText
  ...
  return (
    <Container className='cat-container'>
      ...
      <ul className='categories'>
        {cats.map(cat => <li onClick={showSubCats} key={cat.label} className='category'><a href="#">{cat.icon}</a> <label>{cat.label}</label></li>)}
      </ul>
      <CatPopover placement={'bottom'} labelText={labelText ?? ''} show={showPopOver} target={target}></CatPopover>
    </Container>
  );
};
